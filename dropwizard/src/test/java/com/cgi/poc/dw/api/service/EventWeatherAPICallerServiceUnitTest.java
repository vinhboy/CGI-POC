...
public class EventWeatherAPICallerServiceUnitTest {
  ...
  public void newEventWithNoAffectedUsers() throws IOException {
    ...
    EventWeather eventFromDB = mock(EventWeather.class);
    when(eventDAO.selectForUpdate(any(EventWeather.class))).thenReturn(eventFromDB);
  ...
  public void changedEventWithNoAffectedUsers() throws IOException {
    ...
    EventWeather retEvent = mock(EventWeather.class);
    when(eventDAO.update(any(EventWeather.class))).thenReturn(retEvent);
  ...
  public void logsTheErrorWhenParsingUnexpectedJson() throws IOException {
    ...
    //Check the message being logged is correct
    assertThat(loggingEvent.getFormattedMessage().toLowerCase(),
        containsString("unable to parse the result for the weather event : error:"));
  }
